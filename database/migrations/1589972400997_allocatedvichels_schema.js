...
class AllocatedvichelsSchema extends Schema {
  up () {
    this.create('allocatedvichels', (table) => {
      table.increments()
      table.integer('journey_id').unsigned().references('id').inTable('journies').onDelete('cascade')
      table.integer('vichel_id').unsigned().references('id').inTable('vichels').onDelete('cascade')
